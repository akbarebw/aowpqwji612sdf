...
<div class="row">
    <div class="col-xl-12 col-xxl-12 col-sm-12">
        <div class="card">
            <div class="card-body">
                <form action="{{ route('dashboard.pengaturan.komponen.update', $pengaturan->id) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="col-md-12">
                        <div class="form-group">
                            ...
                            <select name="jenis" id="jenis" class="form-control">
                                ...
                                <option value="teori" {{ $pengaturan->jenis == 'teori' ? 'selected' : '' }}>Teori</option>
                                <option value="praktek" {{ $pengaturan->jenis == 'praktek' ? 'selected' : '' }}>Praktek</option>
                            </select>
                        </div>
                    </div>
                                ...
                                <tr>
                                    <td>
                                        <select name="komponen_id[]" class="form-control select2">
                                            ...
                                            @foreach ($listKomponen as $item)
                                                <option value="{{ $item->id }}" {{ $komponen->id == $item->id ? 'selected' : '' }}>{{ $item->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td>
                    ...
                    <div class="col-sm-12">
                        <div class="form-group">
                            ...
                            <input id="bobot_standar" placeholder="Masukan Bobot Standar Setiap Item" type="number" class="form-control" name="bobot_standar" value="{{ $pengaturan->bobot_standar }}">
                        </div>
                    </div>
...
<script type="text/javascript">
    $(document).ready(function () {
        ...
        // Disable the already selected components across all rows
        ...
        $('.table').on('click', '.btn-delete', function() {
            ...
            updateKomponenSelect();  // Update available options after deletion
        });
    });
</script>
@endpush
