{{-- @php
    $controller = \App\Helper\DzHelper::controller();
    $page = $action = \App\Helper\DzHelper::action();
...
<head>
    ...
    {{-- @if(!empty(('dz.publiconfigc.global.css')))
        @foreach(config('dz.public.aset_dashboard.global.css') as $style)
            <link href="{{ asset($style) }}" rel="stylesheet" type="text/css"/>
        @endforeach
    @endif --}}
    <link class="main-css" href="{{ asset('asset_dashboard/css/style.css') }}" rel="stylesheet">
